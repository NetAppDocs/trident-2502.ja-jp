...
| `luksEncryption` | LUKS暗号化を有効にします。を参照してください link:../trident-reco/security-luks.html["Linux Unified Key Setup（LUKS；統合キーセットアップ）を使用"]。LUKS暗号化はNVMe/TCPではサポートされません。 | "" ASA R2の場合はに設定します `false`。 
