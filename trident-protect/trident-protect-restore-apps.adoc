...
NOTE: Kopia をデータ ムーバーとして使用してバックアップを復元する場合、オプションで CR で注釈を指定するか、CLI を使用して Kopia が使用する一時ストレージの動作を制御できます。参照 https://kopia.io/docs/getting-started/["Kopiaドキュメント"^]設定できるオプションの詳細については、こちらをご覧ください。使用 `tridentctl-protect create --help`Trident Protect CLI で注釈を指定する方法の詳細については、コマンドを参照してください。
